...
## Contract PDF
When a customer's application gets accepted the customer receives an email with contract in the form of a pdf document. This pdf document includes insurance services containing basic insurance as well as additional insurances the customer applied for. The price break up for each insurance selected and the total price of the insuranc e can be viewed in the document as well. PDF is generated via Eledo and the procedure for creating a pdf and how it works can be viewed in chapter "Integration".
...
# Data Integration
For a smooth process several tools were used to integrate Email, Webhooks, PDF-genereator etc. A complete documentation about the APIs and fields used are given below:
